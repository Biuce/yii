...
class Menu extends \common\models\Menu
{
    ...
    public static function checkRule($rule) {
    	//超级管理员允许访问任何页面
    	if(Yii::$app->params['admin'] == Yii::$app->admin->id) {
    		return true;
    	}

    	//rbac
    	if (!Yii::$app->admin->can($rule)) {
    		return false;
    	}
    	return true;
    }
     ...
     * ---------------------------------------
     */
    public static function getBreadcrumbs($rule = '/index/index/index') {
    	//高亮当前栏目及其所有父栏目
     ...
     * ---------------------------------------
     */
    public static function getMenus($rule = '/index/index/index') {
    	$menus = [];
    	...
    	foreach ($menus['main'] as $key => $item) {
    		...
    		if ($nav[0]['id'] == $item['id']) {
    			...
    			if ($second_menu && is_array($second_menu)) {
    				foreach ($second_menu as $key => $check_menu) {
    					if (!static::checkRule($check_menu['url'])) {
    						unset($second_menu[$key]);
    						continue;
    					}
    				}
