...
?>
<?php echo html::beginForm(Url::toRoute(['/admin/admindolog/index']), 'GET');?>
        ...
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            <thead>
                <tr class="text-c">
                    ...
                    <th width="80">ID</th>
                    <th width="80">名称</th>
                    <th width="100">操作用户ID</th>
                    <th width="100">操作用户名</th>
                    <th width="17%">操作控制器</th>
                    <th width="120">操作动作</th>
                ...
                <?php if(is_array($result)&&!empty($result)) :?>
                    <?php foreach($result as $k => $v) :?>
                        <tr class="text-c">
                            ...
                            <td><?= Html::encode($v->controller)?></td>
                            <td><?= Html::encode($v->action)?></td>
                            <td><?= Html::encode($v->ip)?></td>
                            <td><?= Html::encode(date('Y-m-d H:i:s', $v->time))?></td>
                            <td>
                                <?php if(in_array('/admin/admindolog/delete', $power)):?>
                                    <a title="删除" href="javascript:;" onclick="admindolog_del(this,'<?php echo $v->id; ?>')" class="ml-5" style="text-decoration:none">
...
/*菜单-删除*/
function admindolog_del(obj,id){
    layer.confirm('确认要删除吗？',function(index){
        //此处请求后台程序，下方是成功后的前台处理……
        var base_url = "<?php echo Url::to(['/admin/admindolog/delete']); ?>";
