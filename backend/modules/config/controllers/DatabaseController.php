...
class DatabaseController extends BaseController
{
    ...
    public function init(){
        parent::init();
        // var_dump(Yii::$app->params['web']);die;
    ...
    public function actionOptimize($tables = null){
        ...
        if($tables) {
            if(is_array($tables)){
                ...
                $list = Yii::$app->db->createCommand("OPTIMIZE TABLE `{$tables}`")->execute();
                if($list){
                    $this->success("数据表'{$tables}'优化完成！", $this->getForward());
                } else {
    ...
    public function actionRepair($tables = null){
        ...
        if($tables) {
            if(is_array($tables)){
                ...
                $list = Yii::$app->db->createCommand("REPAIR TABLE `{$tables}`")->execute();
                if($list){
                    $this->success("数据表'{$tables}'修复完成！", $this->getForward());
                } else {
    ...
    public function actionDel($time = 0){
        if($time){
            ...
            if(count(glob($path))){
                $this->success('备份文件删除失败，请检查权限！', $this->getForward());
            } else {
                $this->success('备份文件删除成功！', $this->getForward());
            }
        } else {
            $this->error('参数错误！');
        }
    ...
    public function actionExport(){
        ...
        if(Yii::$app->request->isPost && !empty($tables) && is_array($tables)){ //初始化
            ...
            if(is_file($lock)){
                echo json_encode(['status'=> -1, 'info'=>'检测到有一个备份任务正在执行，请稍后再试！' ,'data'=>[]]);
                exit;
            ...
            if(false !== $Database->create()){
                ...
                echo json_encode(['status'=> 1, 'info'=>'初始化成功！' ,'data'=>['tables' => $tables, 'tab' => $tab]]);
                exit;
                // $this->success('初始化成功！', '', array('tables' => $tables, 'tab' => $tab));
            } else {
                echo json_encode(['status'=> -1, 'info'=>'初始化失败，备份文件创建失败！' ,'data'=>[]]);
                exit;
            ...
            if(false === $start){ //出错
                echo json_encode(['status'=> -1, 'info'=>'备份出错！' ,'data'=>[]]);
                exit;
                // $this->error('备份出错！');
            } elseif (0 === $start) { //下一表
                if(isset($tables[++$id])){
                    $tab = array('id' => $id, 'start' => 0);
                    echo json_encode(['status'=> 1, 'info'=>'备份完成！' ,'data'=>['tab' => $tab]]);
                    exit;
                    ...
                    echo json_encode(['status'=> 1, 'info'=>'备份完成！' ,'data'=>[]]);
                    exit;
                    // $this->success('备份完成！');
                }
            } else {
                ...
                echo json_encode(['status'=> 1, 'info'=>"正在备份...({$rate}%)" ,'data'=>['tab' => $tab]]);
                exit;
                // $this->success("正在备份...({$rate}%)", '', array('tab' => $tab));
            }

        } else { //出错
            echo json_encode(['status'=> -1, 'info'=>'参数错误！' ,'data'=>[]]);
            exit;
            // $this->error('参数错误！');
        }
    }
