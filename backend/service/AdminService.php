...
class Adminservice
{
	...
	{
		$adminInfo = Admin::findOne(['adminuser' => $data['adminuser'], 'status' => 1]);
		if($adminInfo['adminid'] == '') return ['status' => 1002, 'msg' => '用户名不存在！'];
        $adminInfo = $adminInfo->toArray();
		$password = Admin::encryPassword($adminInfo['codes'], $data['adminpass']);
		// echo $password;die();
		if($password != $adminInfo['adminpass'])
		{
